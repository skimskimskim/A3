...
//  delay_SC=300001; //reset e2e latency
  ...
  //printf("sent msg: %s \n",buf);
printf("sent msg: %u %u %u %u %u %u %d %d %d %d %d %d 0 %d %d %d %u %u %u %u %u \n", seq_id, reply, asn_time2, delay_SC, rpl_get_parent_ipaddr(instance->current_dag->preferred_parent)->u8[sz-2], rpl_get_parent_ipaddr(instance->current_dag->preferred_parent)->u8[sz-1],    mac_tx_up_ok_counter,  mac_tx_up_error_counter, mac_tx_down_ok_counter, mac_tx_down_error_counter, tsch_queue_overflow, rpl_get_parent_link_stats(instance->current_dag->preferred_parent)->etx, dc_radio, dc_tx, dc_listen, num_pktdrop_queue, num_pktdrop_mac, num_pktdrop_rpl, num_dis+num_dio+num_dao+num_dao_ack, num_parent_switch);
  delay_SC=300001; //reset e2e latency
...
//----------------------------------------------------------------------
        printf("\ncollect info  (%u)\n\n", currSFID);
//it->c_tx_attempt_ewma

        //rpl_instance_t *instance =rpl_get_default_instance();
        if(instance!=NULL && instance->current_dag!=NULL && instance->current_dag->preferred_parent!=NULL){
          linkaddr_t *addr = rpl_get_parent_lladdr(instance->current_dag->preferred_parent);
          if(addr != NULL){

             printf("col %u %u %u %u %u %u %u rx %.2f %u tx %.2f %u\n", linkaddr_node_addr.u8[LINKADDR_SIZE-2],linkaddr_node_addr.u8[LINKADDR_SIZE-1], addr->u8[LINKADDR_SIZE-2], addr->u8[LINKADDR_SIZE-1], (uint16_t)(100*p_tx_attempt_ewma), 0, (uint16_t)(100*p_rx_attempt_ewma), p_num_rx_ewma, p_num_rx_cur,  p_num_tx_ewma, p_num_tx_cur);
